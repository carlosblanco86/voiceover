...
async function mostrarMenu() {
  ...
  console.log('3. ' + colors.yellow('Mostrar y reproducir archivos de audio'));
  console.log('4. ' + colors.yellow('Ver baseDeDatos.txt'));
  console.log('5. ' + colors.red('Salir'));
}

// Ejecuta el archivo que contiene la opción seleccionada, si no funciona lanza un error
    ...
    switch (opcion) {
      ...
      case '2':
        ...
        case '3':
          try {
            const directorioAudios = './audios';
            await fs.access(directorioAudios, fs.constants.R_OK);
            
            const archivos = await fs.readdir(directorioAudios);
        
            if (archivos.length === 0) {
              console.log(colors.red('No hay archivos de audio en el directorio.'));
            } else {
            ...
            if (error.code === 'ENOENT') {
              console.log(colors.red('La carpeta "audios" no existe. Volviendo al menú principal.'));
            } else {
              console.log(colors.red('Error al leer el directorio de audios:', error.message));
            }
          }
          break;
      ...
      case '4':
        try {
          await fs.access('./baseDeDatos.txt', fs.constants.R_OK);
          await verBaseDeDatos();
        } catch (error) {
          console.log(colors.red('El archivo "baseDeDatos.txt" no existe. Volviendo al menú principal.'));
        }
        break;
      case '5':
        rl.close();
